...
def classify_points_straight_curve_corner(
    pts_mm,
    closed=True,
    lookback_mm=1.7,          # كان 1.0
    straight_angle_deg=8.0,
    corner_angle_deg=35.0,
):
    """
    Robust classifier.

    Instead of comparing only 3 points,
    fit a line on a neighborhood and
    measure the maximum deviation.

    Straight:
        all nearby points lie close to one line.

    Curve:
        smooth but not straight.
    ...
    for i in range(n):
        ...
        if len(ss) >= 2:
            linearity = ss[0] / (ss[1] + 1e-9)
        else:
            linearity = 9999.0

        # distance from every point to line
        ...
        # إذا الـ PCA متأكد أنه خط، اعتبره مستقيم
        # حتى لو الزوايا المحلية متذبذبة بسبب البكسلة.
...
def detect_corners_by_lookback(
    pts_mm,
    closed=True,
    lookback_mm=1.7,
    corner_angle_deg=35.0,
    ...
    """
    Find REAL corners only, using the exact same distance-based lookback
    window as classify_points_straight_curve_corner() (so it is exposed to
    the same pixel-staircase noise and is just as robust to it).

    Unlike the point-by-point classifier, this function is never asked to
    decide "straight vs curve" -- it only answers "is this a corner?", which
    is a much easier and more stable question on noisy raster data.

    Returns a sorted list of corner indices (always at least one entry).
    Neighbouring indices that clearly belong to the same physical corner
    (i.e. within half a lookback window of each other) are collapsed to a
...
def classify_points_by_segments(
    pts_mm,
    closed=True,
    lookback_mm=1.7,
    corner_angle_deg=35.0,
    straight_linearity_min=15.0,
    straight_dev_tol_mm=None,
):
    """
    Segment-based classifier.

    Contour -> real corners -> segments between corners -> ONE PCA / ONE
    decision per whole segment, instead of a local straight/curve vote at
    every single point.

    This is the fix for the exact symptom you're seeing: a geometrically
    straight edge that raster-extraction turned into a tiny staircase gets
    chopped into several short "straight" runs (4, 6, 8, 12, 16...) by the
    point-by-point classifier, because each point only "sees" its own
    lookback window and can flip decision on a 1-2 point wobble. Judged as
    ONE segment, that same staircase is still overwhelmingly linear (high
    linearity ratio, small max deviation relative to segment length), so it
    gets tagged "straight" as a whole and collapses cleanly to 2 points.

    Curves/circles are unaffected: a segment with no real corner in it (or
    that isn't linear when judged as a whole) is tagged "curve" and handed
    to DPHull untouched, exactly as before.
    """
    pts = np.asarray(pts_mm, dtype=float)
    n = len(pts)
    if n < 5:
    ...
    corners = detect_corners_by_lookback(
        pts_mm, closed=closed, lookback_mm=lookback_mm,
    ...
    for s, e in segments:
        ...
        if len(seg_pts) <= 2:
            ...
            direction = vh[0]
            linearity = ss[0] / (ss[1] + 1e-9) if len(ss) >= 2 else 9999.0
            ...
            tol = (
                straight_dev_tol_mm if straight_dev_tol_mm is not None
                else np.clip(bbox * 0.006, 0.02, 0.05)
            )
...
def regularize_straight_runs(points, tags):
    """
    Project only STRAIGHT runs onto their best-fit line.

    Curves and corners are left untouched.
    """
...
def simplify_offset_paths_with_curve_tags(
    ...
    pixels_per_mm: float = 1.0,
    min_segment_mm: float = 0.02,
    lookback_mm: float = 1.7,
    straight_angle_deg: float = 5.0,
    corner_angle_deg: float = 35.0,
    classifier: str = "segment",   # "segment" (new, default) or "pointwise" (old, kept for A/B + rollback)
    verbose: bool = True,
    ...
    for idx, ring in enumerate(offset_paths):
        ...
        if classifier == "segment":
            point_tags = classify_points_by_segments(
        ...
        print(f"\nRing {idx}")
        print(Counter(point_tags))
        print(point_tags[:120])
        
        print(Counter(point_tags))
        contours.append(Contour(points=pts, closed=True, is_hole=False,
                                 contour_id=idx, metadata=point_tags))

    # tag-aware pass: "straight" runs collapse to exactly 2 points,
    # "curve"/"corner" runs are left alone (so G2/G3 fitting downstream
    # still has the real arc point-cloud to work with) instead of running
    # one global-epsilon DPHull over the whole ring.
    simplified, reports = simplify_pipeline_by_tags(
        contours, curve_epsilon_mm=epsilon_mm, pixels_per_mm=pixels_per_mm,
        min_segment_mm=min_segment_mm,
    )
