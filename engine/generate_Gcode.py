...
def _arc_geometry(start_pt, center_pt, end_pt, clockwise):
    """
    Given the start, center, and end points of a G2/G3 arc, returns:
      - arc_length_mm: the TRUE geometric length along the arc (radius * swept angle),
        which is always >= the straight-line chord distance and can differ substantially
        for wide sweeps.
      - extreme_points: a list of (x, y) points including the start, end, and any
        cardinal points (0/90/180/270 deg) the arc sweeps through. This is required for
        an accurate bounding box, since an arc can bulge outward past the straight line
        connecting its two endpoints -- using only start/end would UNDER-report the
        true stock size needed, which is a real safety concern for the operator.
    """
    ...
    # k range is computed dynamically from [lo, hi] rather than a fixed -2..2,
    # since a single-revolution range (-pi..pi) is not enough to cover wide
    # sweeps (e.g. a 270 deg arc can span angles like -3*pi/2) -- a fixed
    # range silently drops real extreme points, which under-reports the true
    # bounding box.
...
def _update_bbox(stats, x, y, z=None):
    """Expands the running bounding box to include the given point."""
...
def _new_stats():
    """Initializes the running statistics accumulator used during generation."""
    return {
        "total_points_input": 0,
        "total_points_valid": 0,
        "linear_segments": 0,
        "arc_segments": 0,
        "plunge_moves": 0,
        "cutting_length_mm": 0.0,
        "plunge_length_mm": 0.0,
        # Rapid travel is split into XY and Z on purpose:
        #   - rapid_xy_length_mm is pure X/Y positioning travel (moving from the
        #     end of one path/pass to the start of the next). This is the number
        #     that actually reflects how good the path-ordering (TSP/genetic)
        #     algorithm is -- a bad order means longer XY hops.
        #   - rapid_z_length_mm is the safe_z/retract_z lifts. This is fixed by
        #     (num_paths * num_passes) and says NOTHING about ordering quality,
        #     so it must not be mixed into an "algorithm quality" claim.
        "rapid_xy_length_mm": 0.0,
        "rapid_z_length_mm": 0.0,
...
def _warn(stats, message):
    """Logs a warning AND records it on the stats accumulator so the final
    report can carry a warnings list without the caller having to parse logs."""
    logging.warning(message)
    stats["warnings"].append(message)


def _record_error(stats, message):
    """Same as _warn but for genuine processing errors (Exception caught mid-path).
    Logged at ERROR severity (distinct from soft warnings) but STILL surfaced in
    the report's warnings list -- a caught exception that never reaches the report
    is a real visibility gap: the researcher/operator would have no way to know a
    path silently failed unless they were also watching the console log."""
...
def _estimate_machining_time(stats, feed_rate, plunge_rate, rapid_speed, dwell_seconds):
    """
    Machining Estimator: converts accumulated distances into an estimated total
    machining time. NOTE: the rapid (G0) portion is inherently an ESTIMATE, since
    G-code does not encode a rapid speed -- it depends on the specific machine's
    controller configuration. rapid_speed is a caller-supplied assumption, clearly
    labeled as such in the final report so it isn't mistaken for a guaranteed figure.
    """
    ...
    return {
        ...
        "plunge_time_min": round(plunge_time_min, 3),
        "rapid_time_min_estimated": round(rapid_time_min, 3),
        "dwell_time_min": round(dwell_time_min, 3),
        "assumed_rapid_speed_mm_per_min": rapid_speed,
        "total_time_min": round(total_min, 3),
...
def _estimate_material_removal(stats, num_passes, total_depth_mm, tool_diameter_mm, stepover_mm=None):
    """
    Rough removed-material volume for the cut geometry.

    Uses `stepover_mm` (the actual spacing between adjacent offset/pocket passes)
    as the effective swept width when provided -- this is the CORRECT choice for
    pocket-clearing / offset-fill toolpaths (parallel or concentric offset loops
    with step_over < tool diameter), because adjacent passes overlap: using the
    FULL tool_diameter as the width there double-counts the overlapped strip and
    can overestimate volume by roughly (1 / step_over_ratio) -- e.g. ~1.67x too
    high at a 0.6 step-over ratio.

    Falls back to `tool_diameter_mm` when stepover_mm isn't given -- appropriate
    ONLY for a single, non-overlapping contour/engraving cut. Always pass
    stepover_mm when the input paths come from a pocket/offset-fill stage.

    `cutting_length_mm` already sums the path length ONCE PER DEPTH PASS (the
    geometry is retraced at every step-down), so we divide by num_passes first
    to recover the single-pass path length before computing volume.
    """
    ...
    return {
        ...
        "effective_width_used_mm": round(effective_width_mm, 3),
        "basis": "stepover (pocket/offset-fill)" if stepover_mm else "tool diameter (single-contour assumption)",
...
def _estimate_mrr(feed_rate, step_down, tool_diameter_mm, warning_threshold_mm3_per_min=5000.0):
    """
    Peak Material Removal Rate (mm^3/min) during an active cutting move, assuming
    FULL tool-width engagement. This intentionally always uses tool_diameter_mm
    (not stepover) even for overlapping pocket passes: the worst-case chip load
    a fresh boundary/edge pass can see IS full engagement, and a safety warning
    should use the conservative (worst-case) figure, not the average.

    This is a PEAK figure, not the job's average MRR (much lower once rapids,
    plunges, and dwell are included). warning_threshold_mm3_per_min is a
    heuristic, not a physical law -- tune it per material/tool/spindle.
    """
    if tool_diameter_mm is None:
        return None
    mrr = feed_rate * step_down * tool_diameter_mm
    return {
        "mrr_mm3_per_min": round(mrr, 1),
        "warning_threshold_mm3_per_min": warning_threshold_mm3_per_min,
        "high_mrr_warning": mrr > warning_threshold_mm3_per_min,
    }


def _estimate_cost(total_time_min, machine_hourly_rate):
    """Simple linear machine-time cost estimate. Returns None if no rate given
    (better than silently reporting a cost based on a made-up default rate)."""
...
def _estimate_file_size(gcode_text, large_file_threshold_bytes=1_000_000):
    """
    Actual size of the generated text, measured directly rather than
    approximated from a "bytes per line" guess (which drifts a lot depending
    on coordinate precision and comment density).
    """
    ...
    return {
        ...
        "large_file_warning": size_bytes > large_file_threshold_bytes,
    }


def _finalize_bbox(stats):
    """Converts the running min/max accumulator into a clean, reportable bounding box."""
    bmin, bmax = stats["bbox_min"], stats["bbox_max"]
    if math.isinf(bmin[0]):
        return None  # no geometry was ever processed
    return {
        "x_min": round(bmin[0], 3), "x_max": round(bmax[0], 3),
        "y_min": round(bmin[1], 3), "y_max": round(bmax[1], 3),
        "z_min": round(bmin[2], 3), "z_max": round(bmax[2], 3),
        "width_mm": round(bmax[0] - bmin[0], 3),
        "depth_mm": round(bmax[1] - bmin[1], 3),
        "height_mm": round(bmax[2] - bmin[2], 3),
    }


def _build_report_comment_block(report):
    """Formats the evaluation report as a G-code comment block for the file header."""
    bbox = report["bounding_box"]
    bbox_line = (
        f"X[{bbox['x_min']:.2f}, {bbox['x_max']:.2f}] "
        f"Y[{bbox['y_min']:.2f}, {bbox['y_max']:.2f}] "
        f"Z[{bbox['z_min']:.2f}, {bbox['z_max']:.2f}]"
        if bbox else "N/A (no geometry processed)"
    )
    ...
    lines = [
        ...
        f"{report['segments']['arc']} arc (G2/G3) )",
        f"( Cutting Length: {report['lengths_mm']['cutting']:.2f} mm | "
        f"Rapid XY (ordering): {report['lengths_mm']['rapid_xy']:.2f} mm | "
        f"Rapid Z (safety lifts): {report['lengths_mm']['rapid_z']:.2f} mm )",
        f"( Est. Machining Time: {mt['total_time_formatted']} "
        f"(assumes rapid @ {mt['assumed_rapid_speed_mm_per_min']:.0f} mm/min) )",
    ...
    if report.get("material_removal"):
        ...
        lines.append(f"( Est. Material Removed: {mr['volume_cm3']:.2f} cm^3 "
                      f"[basis: {mr['basis']}] )")
    if report.get("mrr"):
        mrr = report["mrr"]
    ...
    if report.get("cost"):
        c = report["cost"]
        lines.append(f"( Est. Machining Cost: {c['estimated_cost']:.2f} "
                      f"(@ {c['machine_hourly_rate']:.2f}/hr) )")
    lines.append("(--------------------------------------------------)")
    return lines


def _generate_gcode_core(
    optimized_paths,
    ...
    stats = _new_stats()
    # Cursor tracks the tool's assumed real position through the whole program,
    # used to compute travel distances for the report (independent of the
    # G-code text itself, so it never affects what gets written to the file).
    cursor = [0.0, 0.0, 0.0]
    ...
    def _finish(body_lines, processed_count, total_paths):
        ...
        report = {
            ...
            "paths": {
                ...
                "skipped": total_paths - processed_count,
            },
            "points": {"input": stats["total_points_input"], "valid": stats["total_points_valid"]},
            "segments": {
                "linear": stats["linear_segments"],
                "arc": stats["arc_segments"],
                "plunge_moves": stats["plunge_moves"],
            },
            "lengths_mm": {
                "cutting": round(stats["cutting_length_mm"], 3),
                "plunge": round(stats["plunge_length_mm"], 3),
                "rapid_xy": round(stats["rapid_xy_length_mm"], 3),
                "rapid_z": round(stats["rapid_z_length_mm"], 3),
            },
            "machining_time_estimate": mt,
            "bounding_box": _finalize_bbox(stats),
            "material_removal": _estimate_material_removal(
                stats, num_passes, total_depth, tool_diameter_mm, stepover_mm
            ),
            "mrr": _estimate_mrr(feed_rate, step, tool_diameter_mm),
            "cost": _estimate_cost(mt["total_time_min"], machine_hourly_rate),
            "warnings": list(stats["warnings"]),
            # Placeholder so _build_report_comment_block can run once to measure
            # the real file size below (the header reports its own size, so a
            # single pass can't know it in advance).
            "file_size": {"size_bytes": 0, "size_kb": 0.0, "large_file_warning": False},
        }
    ...
    for path_idx, path in enumerate(optimized_paths):
        try:
            ...
            if len(valid_points) < 2:
                _warn(stats, f"Path #{path_idx + 1} skipped: fewer than 2 valid points "
            ...
            for i, pt in enumerate(valid_points):
                ...
                if key in point_to_idx:
                    _warn(stats, f"Path #{path_idx + 1}: duplicate/near-duplicate point at index {i} "
                ...
                while idx < len(valid_points) - 1:
                    ...
                    if idx in arc_map:
                        ...
                        end_pt, center, start_pt, cmd = (
                            arc.get("end"), arc.get("center"), arc.get("start"), arc.get("command", "G1")
                        )
                        if (_is_valid_point(end_pt) and _is_valid_point(center)
                                and _is_valid_point(start_pt) and cmd in ["G2", "G3"]):
...
def _sanitize_core_params(safe_z, retract_z, cut_depth, step_down, feed_rate,
                           plunge_rate, spindle_speed, rapid_speed):
    try:
...
def generate_gcode(
    ...
    """
    Returns G-code text only. Use generate_gcode_with_report() for the analytics
    report too.

    tool_diameter_mm: enables the Peak MRR safety check, and (as a fallback)
        the material-removal volume estimate.
    stepover_mm: the ACTUAL spacing between adjacent offset/pocket passes
        (e.g. tool_diameter_mm * step_over_ratio). Pass this whenever the
        input paths come from a pocket-clearing / offset-fill stage -- it
        makes the material-removal volume estimate correct instead of
        overestimated (see _estimate_material_removal docstring).
    """
    params = _sanitize_core_params(safe_z, retract_z, cut_depth, step_down,
                                    feed_rate, plunge_rate, spindle_speed, rapid_speed)
    text, _report = _generate_gcode_core(
        optimized_paths, *params, tool_diameter_mm, stepover_mm, machine_hourly_rate
    )
    return text


def generate_gcode_with_report(
    optimized_paths,
    safe_z=5.0, retract_z=1.0, cut_depth=-3.0, step_down=1.0,
    feed_rate=800, plunge_rate=300, spindle_speed=12000, rapid_speed=3000.0,
    tool_diameter_mm=None, stepover_mm=None, machine_hourly_rate=None,
...
def _resolve_user_settings(user_settings):
    defaults = {
        "safe_z": 5.0, "retract_z": 1.0, "cut_depth": -3.0, "step_down": 1.0,
        "feed_rate": 800.0, "plunge_rate": 300.0, "spindle_speed": 12000,
        "rapid_speed": 3000.0, "tool_diameter_mm": None, "stepover_mm": None,
    ...
    return dict(
        safe_z=safe_z, retract_z=retract_z, cut_depth=cut_depth, step_down=step_down,
        feed_rate=feed_rate, plunge_rate=plunge_rate, spindle_speed=spindle_speed,
        rapid_speed=rapid_speed, tool_diameter_mm=tool_diameter_mm, stepover_mm=stepover_mm,
        machine_hourly_rate=machine_hourly_rate,
    )


def generate_gcode_from_user_input(optimized_paths, user_settings: dict = None):
    """UI-facing wrapper: text only."""
...
def print_gcode_report(report: dict):
    ...
    print("\n🛣️  PATHS STATUS:")
    print(f"  • Input Paths:     {paths.get('input', 0)}")
    print(f"  • Processed Paths: {paths.get('processed', 0)}")
    print(f"  • Skipped Paths:   {paths.get('skipped', 0)}")

    pts = report.get("points", {})
    seg = report.get("segments", {})
    print("\n📍 POINTS & SEGMENTS:")
    print(f"  • Valid Points:    {pts.get('valid', 0)} / {pts.get('input', 0)}")
    print(f"  • Linear Seg (G1): {seg.get('linear', 0)}")
    print(f"  • Arc Seg (G2/G3): {seg.get('arc', 0)}")
    print(f"  • Plunge Moves:    {seg.get('plunge_moves', 0)}")

    lengths = report.get("lengths_mm", {})
    print("\n📏 TRAVEL DISTANCES (mm):")
    print(f"  • Cutting Length:  {lengths.get('cutting', 0):,.2f} mm")
    print(f"  • Plunge Distance: {lengths.get('plunge', 0):,.2f} mm")
    print(f"  • Rapid XY (TSP):  {lengths.get('rapid_xy', 0):,.2f} mm")
    print(f"  • Rapid Z (Lifts): {lengths.get('rapid_z', 0):,.2f} mm")

    mt = report.get("machining_time_estimate", {})
    if mt:
        print("\n⏳ TIME ESTIMATION:")
        print(f"  • Cutting Time:    {mt.get('cutting_time_min', 0)} min")
        print(f"  • Plunge Time:     {mt.get('plunge_time_min', 0)} min")
        print(f"  • Rapid Time (Est):{mt.get('rapid_time_min_estimated', 0)} min")
        print(f"  • Total Time:      {mt.get('total_time_formatted', 'N/A')}")

    bbox = report.get("bounding_box")
    print("\n📦 BOUNDING BOX (Stock Size Needed):")
    if bbox:
        print(f"  • X Bounds:  [{bbox['x_min']} mm  to  {bbox['x_max']} mm] (Width: {bbox['width_mm']} mm)")
        print(f"  • Y Bounds:  [{bbox['y_min']} mm  to  {bbox['y_max']} mm] (Depth: {bbox['depth_mm']} mm)")
        print(f"  • Z Bounds:  [{bbox['z_min']} mm  to  {bbox['z_max']} mm] (Height: {bbox['height_mm']} mm)")
    else:
        print("  • N/A (No geometry processed)")

    mr = report.get("material_removal")
    mrr = report.get("mrr")
    if mr or mrr:
        print("\n📋 MATERIAL REMOVAL & MRR:")
        if mr:
            print(f"  • Removed Volume: {mr['volume_cm3']} cm³ ({mr['volume_mm3']} mm³) [basis: {mr['basis']}]")
        if mrr:
            print(f"  • Peak MRR:       {mrr['mrr_mm3_per_min']} mm³/min")
            if mrr.get("high_mrr_warning"):
                print("    ⚠️ WARNING: High Material Removal Rate detected!")

    cost = report.get("cost")
    if cost:
        print("\n💰 COST ESTIMATION:")
        print(f"  • Estimated Cost: ${cost['estimated_cost']} (@ ${cost['machine_hourly_rate']}/hr)")

    fsize = report.get("file_size", {})
    print("\n💾 FILE SIZE:")
    print(f"  • Output Size:     {fsize.get('size_kb', 0)} KB ({fsize.get('size_bytes', 0)} bytes)")
