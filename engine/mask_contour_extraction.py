...
def _contours_from_level_mask_subpixel(distance_mm: np.ndarray, level_mm: float, pixel_to_mm: float, min_path_length_mm: float) -> List[Path]:
    ...
    for rc in raw_contours:
        ...
        if perimeter_px < min_length_px:
            continue
        path = _ensure_closed([(x * pixel_to_mm, y * pixel_to_mm) for x, y in pts_px])
        if len(path) >= 4:
            paths.append(path)
    return paths

def _contours_from_level_mask(level_mask: np.ndarray, pixel_to_mm: float, min_path_length_mm: float) -> List[Path]:
    contours, hierarchy = cv2.findContours(level_mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    paths: List[Path] = []
    min_length_px = min_path_length_mm / pixel_to_mm
    for contour in contours:
        ...
        path = _ensure_closed([(float(point[0]) * pixel_to_mm, float(point[1]) * pixel_to_mm) for point in points])
        if len(path) >= 4:
            paths.append(path)
    return paths
